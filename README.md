...
| 04 | **Equity Research Analyst - Full RAG Pipeline** | A full-stack **RAG (Retrieval-Augmented Generation) pipeline** that ingests live news articles via URL loaders, chunks them using recursive character text splitters, and converts chunks into semantic vector embeddings using HuggingFace sentence-transformers stored in a FAISS index. At query time, the retriever performs similarity search over the vector store to surface the most relevant context, which is then passed to Gemini 2.5 Flash for grounded, source-backed answers — eliminating hallucinations by anchoring the LLM strictly to retrieved document chunks. <br/> <br/> Learned the core concepts of **RAG pipelines**, **vector embeddings**, **document loaders**, and **text splitters**, together in a single pipeline while building the tool that ingests live news articles, chunks and embeds them into a FAISS vector store, and uses Gemini 2.5 Flash to answer queries grounded strictly in the retrieved context.| [→ View](https://github.com/priyanshu09102003/GenerativeAI_HandsON/blob/main/Runnables/runnable_passthrough.py) |
